...
- **本地阅读**：导入 `.txt` 小说文件，自动识别 UTF-8 / GB18030 编码，自动拆分章节，章节列表按每 100 章分页。
- **阅读体验**：方向键或按钮翻页，可调字体大小，阅读进度自动保存，切换章节后自动回到章节顶部。
- **AI 概要**：为单章、当前页或全书缺失章节生成概要，帮助快速回顾剧情。
- **知识图谱**：从章节中抽取实体与关系，支持当前章节、范围、全书批量扫描，断点续传，实体/关系编辑、合并、拆分、删除，以及低置信度复审队列。
- **多模型配置**：同时配置本地 Ollama 与多个兼容 OpenAI 的外部模型，按需求切换。
- **离线扫描器**：在浏览器外批量处理概要或知识图谱任务，支持中断续扫并导回主数据库。
- **数据本地存储**：导入的章节、阅读进度、概要、设置、知识图谱全部持久化到 `~/.novel_reader/novel_reader.sqlite`。
...
1. 打开前端页面，点击导入按钮，选择本地 `.txt` 小说文件。
2. 系统会自动检测文件编码（UTF-8 或 GB18030）。
3. 导入后，小说会被拆分为章节，章节列表按每 100 章分页显示。
4. 点击任意章节即可开始阅读。

### 2. 阅读器

- **翻页**：使用键盘左右方向键，或点击页面上的上一章/下一章按钮。
- **字体大小**：在阅读器设置中调整字体大小。
...
- **复审面板**：在知识图谱页面点击“待复审”按钮打开复审队列。
  - 可按实体/关系筛选。
  - 支持批量选择，标记为已审、忽略、删除或编辑。
- **状态重置**：编辑、合并或拆分实体/关系后，系统会自动重置 `review_status`，以便重新进入复审判断。
