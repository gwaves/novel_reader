...
function printHelp() {
  console.log(`
Novel Reader 生产流水线 TTS 导演脚本工具

用法:
  node production-pipeline/scripts/tts-director.mjs <command> [options]
...
示例:
  ...
  node production-pipeline/scripts/tts-director.mjs --config production-pipeline/config/tts-director.example.json inspect-chapter --book-id <id> --chapter 1
  node production-pipeline/scripts/tts-director.mjs --config production-pipeline/config/tts-director.example.json draft-script --book-id <id> --chapter 1 --allow-partial --limit 2000 --out tmp/tts/<book>/ch001.script.json
  MIMO_API_KEY=... node production-pipeline/scripts/tts-director.mjs --config production-pipeline/config/tts-director.example.json synth --script tmp/tts/<book>/ch001.script.json
`)
}
...
function loadConfig(configPath) {
  ...
  if (!existsSync(path)) {
    throw new Error(`找不到配置文件：${path}\n请先复制 production-pipeline/config/tts-director.example.json 到该路径。`)
  }
  ...
  return {
    ...
    director: {
      defaultNarrator: {
        speaker: '旁白',
        voice: '白桦',
        style: '中文有声书男声旁白，低沉、清晰、平静，保留必要停顿。',
