...
- `production-pipeline/config/mimo-v2.5-tts-voices.json`：MIMO 已确认音色说明。
- `production-pipeline/config/volcengine-seed-tts-2-voices.json`：Seed TTS 2.0 已确认音色说明。
...
- 两个 TTS Provider 都只消费导演脚本，不直接从原始小说猜测角色。
- 音色说明属于 Provider；角色资料和固定选角属于具体书籍及生产任务。
