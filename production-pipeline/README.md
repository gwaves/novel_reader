...
`audio/chapter.mp3` tree or invoke `production-pipeline/scripts/tts-director.mjs
...
- `llm.scheduler`: when present, the full-flow runner treats `llm.concurrency`
  ...
  Audio scheduling maps its share to the TTS director `directorConcurrency` and keeps
