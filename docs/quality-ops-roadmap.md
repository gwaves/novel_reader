...
状态：已完成第一版入口，2026-07-04 已刷新为 AI 原生小说阅读器定位并补充移动端截图。

交付物：

- `docs/product-spec.md`：正式产品功能说明书。
- `README.md`：增加规格、测试与运维规划入口，并突出知识图谱、RAG、多角色音频和 Gateway Android。
...
状态：第一版已建立，见 [测试用例矩阵](test-case-matrix.md)。

目标：从产品规格拆出完整测试设计，先形成文档，再逐步自动化。

状态：第一版已启动，P0 自动化和部分 P1/P2 自动化已落地。
...
状态：第一版 checklist 已建立，见 [系统性 Code Review Checklist](code-review-checklist.md)。
...
状态：第一版 runbook 已建立，见 [运维 Runbook](operations-runbook.md)。
...
状态：第一版发布清单已建立，见 [发布检查清单](release-checklist.md)。
...
- [x] 建立完整测试用例矩阵。
- [x] 标注现有测试覆盖与缺口。
- [x] 选择第一批必须自动化的 P0/P1 用例。
- [x] 制定系统性 Code Review checklist。
- [x] 梳理 Gateway 与 production-pipeline 的运维/验收缺口，并沉淀到 runbook。
- [x] 更新发布检查流程。
- [ ] 继续补齐剩余治理项：运维指标真实 Gateway 执行记录、发布回滚真实演练记录。
- [ ] 将 `OPS-METRIC-001` 与 `OPS-ROLLBACK-001` 从脚本治理推进到真实演练记录。
