...
- 当前 Web 桌面端仍可触发 embedding 生成、搜索，并用当前生成模型基于检索结果生成回答。
- 当前维护的 Gateway Android 移动端不生成书籍、章节或 chunk embedding；它只消费 PC/Gateway 已生成并发布的数据包。

## 移动端支持现状

当前维护的 Android 移动端已切换到 [`../gateway-android-app`](../gateway-android-app)。它通过 Gateway 访问书库、单书 package、RAG/AI 和 MP3 音频，并在本地缓存后支持离线阅读与播放。旧 `../mobile-app` 局域网同步客户端目录已删除；历史实现请查看 Git 记录。
