...
├── mobile-app/             # 独立 Android 移动端 workspace（规划中）
├── src/                    # React 前端
│   ├── main.tsx            # 入口（桌面/移动路由）
│   ├── App.tsx             # 桌面端主 UI
...
独立 Android 移动端的详细计划见 [`../mobile-app/development-plan.md`](../mobile-app/development-plan.md)。PC 端在该架构中是数据生产与同步服务，而不是移动端的实时依赖。手机离开家庭局域网后仍应能阅读和查询已同步数据。

PC 端需要新增的职责：

- 继续负责导入书籍、拆章、生成概要、知识图谱抽取、实体清洗、正文 chunk embedding 和概要 embedding。
- 提供 `/api/mobile/*` 读接口，供移动端在同一局域网内拉取完整书籍数据包。
