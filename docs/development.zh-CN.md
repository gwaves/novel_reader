...
├── gateway-android-app/    # 当前维护的 Gateway Android App
...
- `summary_embeddings`、`chapter_chunk_embeddings` — RAG 搜索使用的概要与正文片段 embedding
- `app_state` — 应用设置与模型配置
...
- 当前 Web 桌面端和 `/mobile` 路由仍可触发 embedding 生成、搜索，并用当前生成模型基于检索结果生成回答。
- 独立 Android 移动端不生成书籍、章节或 chunk embedding；它只同步 PC 端已生成的 embedding 数据，并在本地做离线消费。

## 移动端支持现状

当前维护的 Android 移动端已切换到 [`../gateway-android-app`](../gateway-android-app)。它通过 Gateway 访问书库、单书 package、RAG/AI 和 MP3 音频，并在本地缓存后支持离线阅读与播放。旧 [`../mobile-app`](../mobile-app) 局域网同步客户端已退役，仅作为历史实现和迁移参考保留。

PC/Gateway 数据生产链路仍负责：

- 继续负责导入书籍、拆章、生成概要、知识图谱抽取、实体清洗、正文 chunk embedding 和概要 embedding。
- 提供 `/api/mobile/*` 与 Gateway 发布脚本，供 Gateway 导入完整书籍数据包。
- 导出的移动数据包应包含书籍、章节、概要、知识图谱实体/关系/证据，以及 PC 端已生成的 `summary_embeddings` 和 `chapter_chunk_embeddings`。
- 移动数据包不得包含 LLM API Key、桌面端模型配置里的敏感字段，或不必要的桌面 UI 状态。
- 后续可选支持移动端阅读进度回写，但第一版优先单向同步 PC -> mobile。
...
- `/api/mobile/manifest`：返回服务版本、移动数据包 schema 版本、能力列表和当前时间。
- `/api/mobile/books`：返回书架列表及每本书的章节数、字数、概要覆盖率、图谱覆盖率、embedding 覆盖率和更新时间。
- `/api/mobile/books/:bookId/package`：返回单本书完整移动数据包，供移动端导入本地 SQLite。

移动同步服务的安全要求：

- `/api/mobile/*` 不应在没有 token 的情况下暴露给局域网。
- 当 `NOVEL_READER_API_HOST=0.0.0.0` 时，UI 或日志应提示这是局域网可访问服务。
- 第一版可以使用用户配置的静态同步 token；后续再考虑二维码配对、token 轮换和 HTTPS。

移动数据包的实现建议：

- Phase 1 使用 JSON 数据包验证端到端流程。
- 数据包必须带 `schemaVersion`、`bookId`、`generatedAt` 和内容校验信息。
- 大书或多书同步变慢后，再升级为压缩 JSON、NDJSON 流式包或 SQLite 导出包。
- 导入失败不能破坏移动端已有数据，因此 PC 数据包要保持幂等、完整、可校验。
