...
- **知识图谱**：从章节中抽取实体与关系，支持扫描、重扫预览、raw extraction 重放、实体/关系维护、证据搜索、图谱可视化、JSON/GraphML 导出，以及低置信度复审队列。
...
- **Embedding 模型**：可为 RAG 搜索单独配置 Ollama 或 OpenAI-compatible embedding 模型；外部模型配置可指定独立 embedding model。
