  ...
  it('normalizes Gateway Android update manifests', () => {
    expect(
      normalizeAppUpdateManifest({
        ...
        buildNumber: 235,
        gitCommit: 'abcdef0',
        latestUrl: '/downloads/ai_novel_reader.apk',
        latestFileName: 'ai_novel_reader.apk',
    ...
    expect(normalizeAppUpdateManifest({ versionCode: 2000235, latestUrl: '/downloads/ai_novel_reader.apk' })).toBeNull()
    expect(normalizeAppUpdateManifest({ versionName: '0.2.0', latestUrl: '/downloads/ai_novel_reader.apk' })).toBeNull()
  })
